...
public final class WebPUtils {
    ...
    public static void skip1ByteSilently(InputStream in) throws IOException {
        skip1ByteSilently(in, 1);
    }

    public static void skip1ByteSilently(InputStream in, long n) throws IOException {
        Objects.requireNonNull(in);
        while (n > 0) {
            long ns = in.skip(n);
            if (ns > 0 && ns <= n) {
